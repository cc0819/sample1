            ...
            if (mSelfUserInfo.isCreater() == false) {
                if (action.equals(Util.ACTION_ROOM_CREATE_COMPLETE)) {
                    ...
                    if (mCreateRoomErrorCode == AVError.AV_OK) {
                        ...
                        startActivity(new Intent(ReplayListActivity.this, AvActivity.class)
    ...
    private void initLiveListView() {
        ...
        mListViewLiveList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                String recordId = mChoseLiveVideoInfo.getRecordId();

                Toast.makeText(ReplayListActivity.this, "recordId "+recordId, Toast.LENGTH_SHORT).show();


//                if (mChoseLiveVideoInfo != null && mChoseLiveVideoInfo.getUserPhone().equals(mSelfUserInfo.getUserPhone())) {
//                    Toast.makeText(ReplayListActivity.this, "you can't join a not exist room", Toast.LENGTH_SHORT).show();
//                    return;
//                }
...
//                createRoom(roomNum);
    ...
    private void getReplayVideoInfoList() {
        new Thread(new Runnable() {
            @Override
            public void run() {
//                String response = HttpUtil.PostUrl(HttpUtil.getRecordListUrl, new ArrayList<NameValuePair>());
                ImageUtil tool = new ImageUtil();
                String response = null;
                try {
                    JSONObject object = new JSONObject();
                    object.put("timeLimit", "9999-10-19 21:10:57");
                    response = tool.requestRecordList(object, HttpUtil.getRecordListUrl, "replaydata");
                ...
                Log.d(TAG, "getReplayVideoInfoList response  " + response);
...
//                }
                ...
                Log.d(TAG, "getReplayVideoInfoList response jsonTokener " + jsonTokener.toString());
                try {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        Log.d(TAG, "title:" + jobject.getString("subject") +
                        ...
                        if (jobject.isNull(Util.EXTRA_VIEWER_NUM)) {
                            viewerNum = 0;
                        } else {
                            viewerNum = jobject.getInt(Util.EXTRA_VIEWER_NUM);
                        }
                        ;
                        if (jobject.isNull(Util.EXTRA_PRAISE_NUM)) {
                            praiseNum = 0;
                        } else {
                            praiseNum = jobject.getInt(Util.EXTRA_PRAISE_NUM);
                        }
                        String recordId;
                        if (jobject.isNull(Util.EXTRA_REPLAYID)) {
                            recordId = "";
                        } else {
                            recordId = jobject.getString(Util.EXTRA_REPLAYID);
                        }
                        String recordTime;
                        if (jobject.isNull(Util.EXTRA_RECORDTIME)) {
                            recordTime = "";
                        } else {
                            recordTime = jobject.getString(Util.EXTRA_RECORDTIME);
                        }
